...
void main() {
    ...
    float v = texture(heightTex, texCoord).r * 0.04 + (-0.02);
    ...
    float difCoef = pow(max(0, normalizedLightVec.z), 0.7) * max(0, dot(inNormal, normalizedLightVec));
    vec3 difComponent = directLightCol * matDifCol * difCoef;

    vec3 reflected = reflect(-normalizedLightVec, inNormal);

    float specCoef = pow(max(0, normalizedLightVec.z), 0.7) * pow(max(0, dot(normalize(eyeVec), reflected)), 70);
    vec3 specComponent = directLightCol * matSpecCol * specCoef;

    float spotEffect = max(dot(normalize(spotDir), -normalizedLightVec), 0);
